...
spd_santarem <- spd(            #Generation fo the SPD of Santarém radiocarbon dates
  c14calib_santarem,            #Using the Santarém radiocarbon calibrations
  timeRange = timeRange         #Time Range from 2200 BP to 100 BP 
) 


######################################################################################################
#                                       PLOTTING ALL ANALYSES                                        #
######################################################################################################
...
#Plot XX - Summed Probability Distribution (SPD) for Santarém pottery radiocarbon datings
plot(
  spd_santarem,           #SPD object for Santarém Radiocarbon dates
  calendar = 'BCAD',      #Plot the SPD Santarém Model in BC/AD years
  main = "Summed Probability Distribution (SPD) - Santarém Pottery" #Label
  ) 

plot(                     #Plotting the smoothing line with a bin value of 200 years
  spd_santarem,           #SPD object for all radiocarbon dates
  calendar = 'BCAD',      #Calendar in BC/AD years
  runm = 200,             #Smoothed distribution with a binning of 200 years
  add = TRUE,             #Add the line
  type = "simple",        #Simple line
